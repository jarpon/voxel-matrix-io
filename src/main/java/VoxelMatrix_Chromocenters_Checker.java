...
public class VoxelMatrix_Chromocenters_Checker implements PlugIn{ 
	...
	public void run(String arg) 
	{
		...
		nf.setDialogTitle("Select the folder of your chromocenters images...");
		...
		if (retval == JFileChooser.APPROVE_OPTION && retval2 == JFileChooser.APPROVE_OPTION) {
			...
			try 
			{
				new VoxelMatrixSegmentationChecker(originalFolder, nucleusFolder);
			} 
			catch (IOException e) 
			{
...
class VoxelMatrixSegmentationChecker extends PlugInFrame implements ActionListener{
	...
	{
		super("Segmentation of CC's checker");
	...
	{ 
		...
		if ( hasSameName.equals(nucleusFilesList[counter].getName().toString()) ) {
    		...
    		//ImageWindow iw = new ImageWindow(nucleusIMP);
	...
	void discardImage() throws IOException
	{
		...
		IJ.log("discardedCC's "+discardedFilesList.size()+"/"+counter);
	...
	void finishProcess()
	{
		...
		String discardedFolder = nucleusFilesList[0].getParentFile().getParentFile().toString()+ "/discardedCCs";
